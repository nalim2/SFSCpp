...
Socket::~Socket() {
    ...
    executor.execute(new std::function<void()>([closer = closer] { closer->operator()(); }));
}

void Socket::connect(std::string endpoint) {
    executor.execute(new std::function<void()>(
...
void Socket::setXPubVerbose() {
    executor.execute(new std::function<void()>([zmqSocket = zmqSocket] {
